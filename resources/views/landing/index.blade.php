...
<head>
    ...
    <style>
        ...
        /* Room card */
        .room-card { transition: transform .25s ease, box-shadow .25s ease; }
        .room-card:hover { transform: translateY(-4px); box-shadow: 0 16px 32px rgba(37,99,235,.13); }
        ...
        .fu-1 { animation: fadeUp .5s .1s ease both; }
        .fu-2 { animation: fadeUp .5s .2s ease both; }
        .fu-3 { animation: fadeUp .5s .3s ease both; }
    </style>
</head>
...
{{-- ══════════════════ HERO ══════════════════ --}}
<section class="bg-gradient-to-b from-blue-50 to-white pt-24 sm:pt-28 pb-12 sm:pb-16">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h1 class="text-3xl sm:text-5xl lg:text-6xl font-extrabold leading-tight tracking-tight mb-4 fu-1">
            Kos Nyaman untuk<br>
            <span class="gradient-text">Mahasiswa & Pekerja</span>
        </h1>

        <p class="text-gray-500 text-base sm:text-lg max-w-2xl mx-auto mb-7 leading-relaxed fu-2">
            Fasilitas lengkap, WiFi cepat, kamar mandi dalam & keamanan 24 jam.<br class="hidden sm:block">
            Mulai dari <strong class="text-blue-600">Rp {{ number_format($minPrice / 1000000, 1) }}jt</strong>/bulan.
        </p>

        <div class="flex flex-col sm:flex-row gap-3 justify-center mb-10 fu-3">
            <a href="#kamar" class="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-7 py-3.5 rounded-xl shadow-md hover:shadow-lg transition-all text-sm sm:text-base">
                <i class="fas fa-bed"></i> Lihat Kamar Tersedia
            </a>
            <a href="#kontak" class="inline-flex items-center justify-center gap-2 border-2 border-blue-600 text-blue-600 hover:bg-blue-50 font-semibold px-7 py-3.5 rounded-xl transition-all text-sm sm:text-base">
                <i class="fab fa-whatsapp text-green-500"></i> Hubungi Kami
            </a>
        </div>

        {{-- Hero image --}}
        <div class="rounded-2xl overflow-hidden shadow-2xl fu-3">
            <img src="https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=1200&h=560&fit=crop"
                 alt="Interior Kosan"
                 class="w-full h-52 sm:h-72 lg:h-96 object-cover">
        </div>

        {{-- Stats --}}
        <div class="grid grid-cols-3 gap-3 sm:gap-5 mt-8 max-w-xl mx-auto">
            <div class="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm text-center">
                <div class="text-2xl sm:text-3xl font-extrabold text-blue-600">{{ $availableRooms }}</div>
                <div class="text-gray-500 text-xs sm:text-sm mt-1">Kamar Tersedia</div>
            </div>
            <div class="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm text-center">
                <div class="text-2xl sm:text-3xl font-extrabold text-blue-600">
                    {{ $minPrice > 0 ? number_format($minPrice/1000000,1).'jt' : 'N/A' }}
                </div>
                <div class="text-gray-500 text-xs sm:text-sm mt-1">Mulai /bulan</div>
            </div>
            <div class="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm text-center">
                <div class="text-2xl sm:text-3xl font-extrabold text-blue-600">24/7</div>
                <div class="text-gray-500 text-xs sm:text-sm mt-1">Keamanan</div>
            </div>
        </div>
...
<section id="fasilitas" class="py-14 sm:py-20 bg-white">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-10">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-2">Fasilitas Lengkap</h2>
            <p class="text-gray-500 text-sm sm:text-base">Semua yang kamu butuhkan sudah tersedia</p>
        </div>
        ...
        <div class="sm:hidden h-scroll -mx-4 px-4">
            ...
            <div class="flex flex-col items-center justify-center text-center bg-blue-50 border border-blue-100 rounded-2xl w-28 p-4 gap-2">
                <div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm">
                    <i class="{{ $f['icon'] }} text-blue-600 text-base"></i>
                </div>
        ...
        <div class="hidden sm:grid grid-cols-4 gap-4">
            @foreach($facList as $f)
            <div class="flex flex-col items-center text-center bg-blue-50 border border-blue-100 rounded-2xl p-5 gap-3 hover:shadow-md transition-shadow">
...
<section id="kamar" class="py-14 sm:py-20 bg-gray-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-8">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-2">Daftar Kamar</h2>
            <p class="text-gray-500 text-sm sm:text-base">Pilih kamar yang sesuai kebutuhanmu</p>
        </div>
        ...
        <div class="max-w-2xl mx-auto mb-8 space-y-3">
            <div class="relative">
                ...
                <input id="searchInput" type="text" placeholder="Cari nama kamar..."
                       oninput="searchRooms()"
                       class="w-full pl-11 pr-4 py-3 border border-gray-200 bg-white rounded-xl text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
            </div>
            <div class="flex flex-wrap gap-2 justify-center">
                <button id="btnAvailable" onclick="filterAvailable()"
                        class="filter-btn px-4 py-2 rounded-xl text-sm font-semibold border border-gray-200 bg-white text-gray-600 hover:border-blue-400 hover:bg-blue-50 transition-all">
                    <i class="fas fa-check-circle mr-1.5 text-green-500"></i>Hanya Tersedia
                </button>
                <button onclick="sortRooms('name-asc')"
                        class="px-4 py-2 rounded-xl text-sm font-semibold border border-gray-200 bg-white text-gray-600 hover:border-blue-400 hover:bg-blue-50 transition-all">
        ...
        {{-- Grid --}}
        <div id="roomsContainer" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            @forelse($rooms as $room)
                @php
                    $avg   = $room->reviews->avg('rating') ?? 0;
                    $rcnt  = $room->reviews->count();
                    $cycle = match($room->billing_cycle ?? '') {
                        'daily'  => 'hari', 'weekly' => 'minggu',
                        'monthly'=> 'bulan', default  => 'tahun'
                    };
                @endphp
                   ...
                   data-price="{{ $room->price }}">
                    ...
                    <div class="relative h-44 sm:h-48 overflow-hidden bg-gray-100">
                        ...
                        <div class="absolute top-3 right-3">
                            @if($room->status == 'available')
                                <span class="inline-flex items-center gap-1 bg-green-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                            ...
                            @else
                                <span class="inline-flex items-center gap-1 bg-orange-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                        ...
                        <div class="absolute bottom-3 left-3">
                            <span class="text-xs text-white bg-black/60 px-2.5 py-1 rounded-full backdrop-blur-sm">
                                <i class="fas fa-building mr-1 opacity-70 text-[10px]"></i>{{ $room->property->name }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="p-4 space-y-2.5">
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <h3 class="font-bold text-gray-900 text-base leading-snug line-clamp-1 flex-1">{{ $room->name }}</h3>
                            @if($rcnt > 0)
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            @if($room->facilities->count() > 0)
                                ...
                                <span class="text-xs bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-lg font-medium">
                                ...
                                @endif
                            @else
                                <span class="text-xs bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-lg font-medium"><i class="fas fa-bed mr-1 text-[9px]"></i>Kasur</span>
                                <span class="text-xs bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-lg font-medium"><i class="fas fa-bath mr-1 text-[9px]"></i>K. Mandi</span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center justify-between bg-blue-600 rounded-xl px-4 py-3 mt-1">
                            <div>
                                <div class="text-white font-extrabold text-base leading-none">
                                    Rp {{ number_format($room->price, 0, ',', '.') }}
                                </div>
                                <div class="text-blue-200 text-xs mt-0.5">per {{ $cycle }}</div>
                            </div>
            ...
            @empty
                <div class="col-span-full text-center py-16">
                    ...
                    <a href="https://wa.me/6283841806357"
                       class="inline-flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 transition text-sm font-semibold">
...
<section id="lokasi" class="py-14 sm:py-20 bg-white">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-10">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-2">Lokasi Strategis</h2>
            <p class="text-gray-500 text-sm sm:text-base max-w-lg mx-auto">
                Dekat kampus, minimarket, transportasi, dan area kuliner.
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div class="space-y-3">
                ...
                @endphp
                @foreach($locs as $loc)
                <div class="flex items-center gap-4 p-4 sm:p-5 rounded-2xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-md transition-all">
                    <div class="w-12 h-12 sm:w-14 sm:h-14 {{ $loc['bg'] }} rounded-2xl flex items-center justify-center shrink-0">
                        <i class="{{ $loc['icon'] }} {{ $loc['ic'] }} text-lg sm:text-xl"></i>
                    </div>
                    <div>
                        <div class="font-bold text-gray-900 text-sm sm:text-base">{{ $loc['title'] }}</div>
                        <div class="text-gray-500 text-xs sm:text-sm">{{ $loc['sub'] }}</div>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            {{-- Map --}}
            <div class="rounded-2xl overflow-hidden shadow-lg border border-gray-100 h-64 sm:h-80 lg:h-full min-h-[320px]">
...
{{-- ══════════════════ CTA ══════════════════ --}}
<section id="kontak" class="py-14 sm:py-20 bg-blue-600">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 text-center">
        <h2 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white mb-3">
            Tertarik untuk Ngekos?
        </h2>
        <p class="text-blue-100 text-base sm:text-lg mb-8">
            Hubungi kami sekarang atau jadwalkan kunjungan langsung.
        </p>
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="https://wa.me/6287824660303?text=Halo,%20saya%20ingin%20info%20lengkap%20tentang%20kos"
               class="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-bold px-7 py-3.5 rounded-xl hover:bg-blue-50 transition-all shadow-md text-sm sm:text-base">
                <i class="fab fa-whatsapp text-green-500 text-lg"></i> Chat WhatsApp
            </a>
            <a href="tel:+6283841806357"
               class="inline-flex items-center justify-center gap-2 border-2 border-white/60 text-white font-bold px-7 py-3.5 rounded-xl hover:bg-white/10 transition-all text-sm sm:text-base">
...
<button id="backToTop" onclick="scrollToTop()"
        class="w-12 h-12 rounded-full bg-blue-600 hover:bg-blue-700 text-white shadow-xl flex items-center justify-center transition-colors">
