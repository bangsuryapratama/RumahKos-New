        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Pilih kamar dan fasilitas yang ingin di-assign
                </p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.facility_rooms.store') }}" class="space-y-6">
                
                @csrf

                {{-- Room Selection --}}
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Pilih Kamar</h3>
                    <div class="relative">
                        <select name="room_id" 
                                id="room_id"
                                required
                            ...
                            @foreach($rooms as $r)
                                <option value="{{ $r->id }}" @selected(old('room_id') == $r->id)>
                                    {{ $r->name }} - {{ $r->property->name }} ({{ $r->status == 'available' ? 'Tersedia' : 'Terisi' }})
                                </option>
                            @endforeach
                        </select>
