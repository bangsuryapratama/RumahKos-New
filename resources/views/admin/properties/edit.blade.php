...
<div class="max-w-5xl mx-auto py-6 sm:py-10 px-4">
    ...
    <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Edit Property</h1>
        <p class="text-sm sm:text-base text-gray-600">Perbarui informasi property</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.properties.update', $property->id) }}"
        ...
        <div>
            ...
            <input type="hidden" name="maps_embed" id="maps_embed" value="{{ old('maps_embed', $property->maps_embed) }}">

            <div id="map-preview"
                 class="mt-4 rounded-lg sm:rounded-xl overflow-hidden {{ $property->maps_embed ? '' : 'hidden' }}">
                <iframe id="map-iframe"
                        class="w-full h-48 sm:h-64 border-0"
                        src="{{ $property->maps_embed ? (strpos($property->maps_embed, 'src="') !== false ? substr($property->maps_embed, strpos($property->maps_embed, 'src="') + 5, strpos($property->maps_embed, '"', strpos($property->maps_embed, 'src="') + 5) - strpos($property->maps_embed, 'src="') - 5) : '') : '' }}"
                        loading="lazy"
        ...
        <div>
            ...
            <textarea name="description"
                      ...
                      placeholder="Deskripsi singkat property...">{{ old('description', $property->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Event listener for Enter key or input change
    ...
    searchButton.className = 'absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all text-sm';
    searchButton.innerHTML = '<i class="fas fa-search"></i>';

    const mapSearchContainer = mapSearchInput.parentElement;
    mapSearchContainer.style.position = 'relative';

    searchButton.addEventListener('click', function() {
        updateMap(mapSearchInput.value);
    });
});
</script>
