...
<body class="bg-gradient-to-br from-blue-50 via-white to-cyan-50 min-h-screen flex flex-col overflow-x-hidden">
    ...
    <nav class="relative z-10 flex items-center justify-between px-4 sm:px-8 py-4 border-b border-white/60 bg-white/60 backdrop-blur-md">
        ...
        <a href="{{ url()->previous() }}" class="text-sm font-semibold text-blue-600 hover:text-blue-800 transition flex items-center gap-1">
    ...
    <main class="flex-1 flex items-center justify-center px-4 py-16">
        <div class="text-center max-w-md w-full">
            ...
            <div class="inline-flex items-center justify-center w-20 h-20 bg-purple-50 border-2 border-purple-100 rounded-2xl mb-5">
                <i class="fas fa-clock text-purple-400 text-3xl"></i>
            </div>

            <h1 class="text-2xl sm:text-3xl font-black text-gray-900 mb-3">
                Halaman <span class="gradient-text">expired</span> 😶
            </h1>
            <p class="text-gray-400 text-sm sm:text-base leading-relaxed mb-6">
                Sesi kamu sudah habis atau terlalu lama tidak aktif. Kembali ke halaman sebelumnya dan coba lagi.
            </p>

            <div class="bg-white border border-purple-100 rounded-xl p-4 mb-4 text-left shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <div class="font-bold text-gray-800 text-sm mb-0.5">Error 419 – Page Expired</div>
                        <div class="text-gray-400 text-xs leading-relaxed">Token CSRF kamu sudah kadaluarsa. Kembali dan isi ulang form ya.</div>
                    </div>
                </div>
            </div>

            <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-6 text-sm text-blue-600 font-semibold">
                <i class="fas fa-rotate-right mr-1.5"></i>
                Otomatis redirect dalam <span id="timer" class="font-black text-blue-700">10</span> detik...
            </div>

            <div class="flex flex-col sm:flex-row gap-3 justify-center">
                <a href="{{ url()->previous() }}" class="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-xl shadow-md transition text-sm">
    ...
    <script>
        ...
        const t = setInterval(() => {
            el.textContent = --s;
            if (s <= 0) { clearInterval(t); window.location.href = "{{ url()->previous() }}"; }
        }, 1000);
    </script>
</body>
</html>